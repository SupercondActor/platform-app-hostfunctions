...
_SupercondActor_Context.Logger.logInfo(['Processed Root API call', results]);
return results;`,
            stopRequested: false
        };
...
_SupercondActor_Context.Logger.logInfo(['Test API Service #${runCounter} results', results]);
        return results;`,
            stopRequested: false
        };
        return serviceConfig;
...
let results = await TestManager.runSystemTests(${runCounter});
_SupercondActor_Context.Logger.logInfo(['Test Scheduled Service #${runCounter} results', results]);
return results;`
            }
        };
        return serviceConfig;
...
while (true) {
    let results = await TestManager.runSystemTests(${runCounter});
    _SupercondActor_Context.Logger.logInfo(['Test Long-running Service #${runCounter} results', results]);
    ...
    async callAwailableApis(rootApiRequest: SupercondActor.IApiRequest): Promise<string[]> {
        // determine API's base url from the root API request's headers
        let schema = rootApiRequest.headers.find(h => h.Key == 'X-Forwarded-Proto').Value;
        let host = rootApiRequest.headers.find(h => h.Key == 'X-Forwarded-Host').Value;
        let port = rootApiRequest.headers.find(h => h.Key == 'X-Forwarded-Port').Value;
        let urlPrefix = rootApiRequest.headers.find(h => h.Key == 'X-Forwarded-Prefix').Value;
        let baseUrl = `${schema}://${host}:${port}${urlPrefix}`;
        ...
        apis.forEach(a => {
            ...
            let resPr = axios.get<SupercondActor.IApiServiceInfo>(url);
            apiPrs.push(resPr);
        });
        ...
        testResults.push(`Found ${results.length} available APIs in application ${this.appName}`);
        results.forEach(r => {
